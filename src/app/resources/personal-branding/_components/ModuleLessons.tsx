      ...
      {module.lessons.map((lesson) => {
        ...
        return (
          <Link
            ...
            className="flex items-center gap-3 p-3 rounded-xl hover:bg-[var(--gray-50)] transition-colors group"
          >
            {/* Icon - show checkmark if completed, lock if not accessible, lesson type otherwise */}
            {isLoading ? (
              <div className="w-8 h-8 rounded-full bg-[var(--gray-100)] animate-pulse flex-shrink-0" />
              ...
              <div className="w-8 h-8 rounded-full border-2 border-[var(--card-border)] flex items-center justify-center flex-shrink-0 group-hover:border-[var(--accent)]">
                {lesson.type === "video" ? (
                  <svg className="w-4 h-4 text-[var(--gray-600)] group-hover:text-[var(--accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="w-8 h-8 rounded-full bg-[var(--gray-100)] flex items-center justify-center flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium truncate group-hover:text-[var(--accent)] ${isCompleted(lesson.slug) ? "text-[var(--accent-green-text)]" : ""}`}>
                {lesson.title}
              </p>
