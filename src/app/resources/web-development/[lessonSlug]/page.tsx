...
export async function generateMetadata({ params }: PageProps) {
  ...
  const title = `${lessonInfo.lesson.title} | Web Development | The Black Female Engineer`;
  const description = `Learn about ${lessonInfo.lesson.title} in this beginner-friendly Web Development guide.`;
  return {
    title,
    description,
    openGraph: { title, description, url: `/resources/web-development/${lessonSlug}`, type: "article" },
...
export default async function LessonPage({ params }: PageProps) {
  ...
  const content = getLessonContent("web-development", lessonSlug);
        ...
        <div className="bg-[var(--gray-50)] border-b border-[var(--card-border)]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center gap-2 text-sm text-[var(--gray-600)]">
              ...
              <span>/</span>
              <Link href="/resources/web-development" className="hover:text-[var(--accent)]">
                Web Development
              </Link>
              <span>/</span>
