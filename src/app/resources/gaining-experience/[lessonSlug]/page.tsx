...
export async function generateMetadata({ params }: PageProps) {
  ...
  const title = `${lessonInfo.lesson.title} | Gaining Experience | The Black Female Engineer`;
  const description = `Learn about ${lessonInfo.lesson.title} in this comprehensive guide to gaining experience without traditional experience.`;
  return {
    title,
    description,
    openGraph: { title, description, url: `/resources/gaining-experience/${lessonSlug}`, type: "article" },
...
export default async function LessonPage({ params }: PageProps) {
  ...
  const content = getLessonContent("gaining-experience", lessonSlug);
        ...
        <div className="bg-[var(--gray-50)] border-b border-[var(--card-border)]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center gap-2 text-sm text-[var(--gray-600)]">
              ...
              <span>/</span>
              <Link href="/resources/gaining-experience" className="hover:text-[var(--accent)]">
                Gaining Experience
              </Link>
              <span>/</span>
