...
export async function generateMetadata({ params }: PageProps) {
  ...
  const title = `${lessonInfo.lesson.title} | Interview Prep | The Black Female Engineer`;
  const description = `Learn about ${lessonInfo.lesson.title} in this comprehensive guide to mastering tech interviews.`;
  return {
    title,
    description,
    openGraph: { title, description, url: `/resources/interview-prep/${lessonSlug}`, type: "article" },
...
export default async function LessonPage({ params }: PageProps) {
  ...
  const content = getLessonContent("interview-prep", lessonSlug);
        ...
        <div className="bg-[var(--gray-50)] border-b border-[var(--card-border)]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center gap-2 text-sm text-[var(--gray-600)]">
              ...
              <span>/</span>
              <Link href="/resources/interview-prep" className="hover:text-[var(--accent)]">
                Interview Prep
              </Link>
              <span>/</span>
