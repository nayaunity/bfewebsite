...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="absolute top-2 right-2 p-2 rounded-lg bg-[var(--gray-200)] hover:bg-[var(--gray-300)] text-[var(--gray-600)] hover:text-[var(--foreground)] transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-10"
...
export default function LessonContent({
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    await subscribe(email, {
      tags: ["bfewebsite", "course-signup", "github-essentials-guide"],
      onSuccess: () => {
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="bg-[var(--gray-50)] rounded-2xl p-8 md:p-12">
          <div className="text-center max-w-md mx-auto">
            <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-[var(--cta-bg)] flex items-center justify-center">
            ...
            <form onSubmit={handleUnlock} className="space-y-3">
              <input
                ...
                className="w-full px-5 py-4 border border-[var(--card-border)] rounded-full focus:outline-none focus:border-[var(--accent)] bg-[var(--card-bg)] disabled:opacity-50"
              />
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-[var(--cta-bg)] text-white px-8 py-4 rounded-full font-medium hover:bg-[#d94d25] transition-colors disabled:opacity-50"
              >
        ...
        <div className="mt-12 flex items-center justify-between pt-8 border-t border-[var(--card-border)]">
          {prevLesson ? (
            <Link
              href={`/resources/github-essentials/${prevLesson.slug}`}
              className="flex items-center gap-2 text-[var(--gray-600)] hover:text-[var(--accent)] transition-colors"
            >
          ...
          {nextLesson ? (
            <Link
              href={`/resources/github-essentials/${nextLesson.slug}`}
              className="flex items-center gap-2 text-[var(--gray-600)] hover:text-[var(--accent)] transition-colors"
            >
      ...
      {hasContent ? (
        <>
                  ...
                  <ReactMarkdown
                    components={{
                      code: ({ className, children, ...props }) => {
                        ...
                        if (!isCodeBlock) {
                          return (
                            <code className="bg-[var(--gray-100)] px-1.5 py-0.5 rounded text-[var(--accent)] font-normal text-sm" {...props}>
                              {children}
                            </code>
                          );
                        }
                        return (
                          <code className="text-[var(--foreground)]" {...props}>
                            {children}
                          </code>
                        );
                      },
                      pre: ({ children }) => {
                        ...
                        return (
                          <div className="relative group my-4">
                            ...
                            <pre className="bg-[var(--gray-100)] p-4 pr-12 rounded-lg overflow-x-auto text-sm">
                              {children}
                            </pre>
                          </div>
                        );
                      ...
                      blockquote: ({ children }) => (
                        <blockquote className="border-l-4 border-[var(--accent)] bg-[var(--blockquote-bg)] pl-4 py-3 my-4 italic text-[var(--blockquote-text)]">
                          {children}
                        </blockquote>
                      ),
                    }}
              ...
              <div className="space-y-4">
                {content.resources.map((resource, index) => (
                  <a
                    ...
                    className="block bg-[var(--card-bg)] p-4 rounded-xl border border-[var(--card-border)] hover:border-[var(--accent)] hover:shadow-md transition-all group"
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <h4 className="font-medium mt-2 group-hover:text-[var(--accent)] transition-colors">
                          {resource.title}
                        </h4>
                      ...
                      <svg className="w-5 h-5 text-[var(--gray-200)] group-hover:text-[var(--accent)] transition-colors flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {content.keyTakeaways.length > 0 && (
            <div className="mt-8 p-6 md:p-8 bg-[var(--cta-bg)]/10 border border-[var(--accent)]/30 rounded-2xl">
          ...
          <div className="mt-8 p-6 bg-[var(--gray-50)] rounded-2xl">
            {isSignedIn ? (
              ...
              <div className="text-center">
                ...
                <Link
                  href={`/auth/signin?callbackUrl=/resources/github-essentials/${lesson.slug}`}
        ...
        <div className="text-center py-16">
          ...
          <Link
            href="/resources/github-essentials"
