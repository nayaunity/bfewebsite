  ...
  return (
    <div className="bg-white border border-[#f5e6d3] rounded-xl p-3.5" style={{ boxShadow: "0 8px 24px rgba(0,0,0,0.04)" }}>
      <div className="flex gap-2.5 mb-2.5">
        <div className="w-9 h-9 rounded-lg bg-[var(--cta-bg)]" />
        <div className="flex-1">
          <div className="h-2 w-[60%] bg-[var(--dark-section-bg)] rounded-full" />
    ...
    <div className="flex flex-col gap-1.5">
      {items.map((item, k) => (
        <div key={k} className="bg-white border border-[#f5e6d3] rounded-[10px] px-3 py-2.5 flex items-center gap-2.5" style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
          <div className="w-7 h-7 rounded-md bg-[var(--dark-section-bg)] text-white grid place-items-center text-[11px] font-bold">
            {item.c[0]}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-xs font-semibold text-[var(--foreground)]">{item.r}</div>
...
function ApplyPreview() {
  return (
    <div className="bg-[var(--dark-section-bg)] rounded-xl p-3.5 text-white font-mono text-[11px]">
