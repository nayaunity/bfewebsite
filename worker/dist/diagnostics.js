...
exports.logBrowserBinaryOnce = logBrowserBinaryOnce;
exports.logEgressIpOnce = logEgressIpOnce;
exports.captureFailureSnapshot = captureFailureSnapshot;
 ...
 */
async function logBrowserBinaryOnce() {
    if (loggedBrowserBinary)
        return;
    loggedBrowserBinary = true;
    try {
        const defaultPath = playwright_1.chromium.executablePath();
        let chromiumChannelPath = null;
        try {
            // Newer Playwright supports passing options to executablePath()
            chromiumChannelPath = playwright_1.chromium.executablePath({ channel: "chromium" });
        }
        catch {
...
async function logEgressIpOnce() {
    if (loggedEgressIp)
        return;
    loggedEgressIp = true;
    try {
