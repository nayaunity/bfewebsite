...
async function signupOrSignin(args) {
    const { page, tenant, userId, applicationEmail, steps, deadlineMs } = args;
    const cred = await (0, credentials_js_1.getOrCreateCredential)(userId, tenant.host, applicationEmail);
    ...
    const applied = await clickIfVisible(page.locator(aid("adventureButton")).first(), "Apply (adventureButton)", steps);
    if (!applied) {
        return { success: false, error: "workday-apply-button-not-found" };
    }
    await page.waitForLoadState("networkidle", { timeout: NAV_TIMEOUT_MS }).catch(() => { });
    // Step 2: click "Apply Manually" on the gate.
    const manual = await clickIfVisible(page.locator(aid("applyManually")).first(), "Apply Manually", steps);
    if (!manual) {
    ...
    if (!cred.isNew) {
        const switched = await clickIfVisible(page.locator(aid("signInLink")).first(), "Sign In link (existing creds path)", steps);
        if (!switched) {
    ...
    if (deadlineMs() <= 0)
        return { success: false, error: "workday-budget-exhausted" };
    return await fillSignupForm(args, accountEmail, cred.password);
}
async function fillSignupForm(args, accountEmail, password) {
    const { page, tenant, applicationEmail, steps, deadlineMs } = args;
    const fields = (0, tenants_js_1.fieldMapFor)(tenant);
    ...
    const needsEmailVerify = /verify|confirm/i.test(url) ||
    ...
    if (needsEmailVerify) {
        ...
        const result = await (0, email_verify_js_1.waitForWorkdayVerifyLink)(accountEmail, remainingForVerify);
        if (!result.link) {
            return {
                success: false,
    ...
    if (hasCreateForm || hasSigninForm) {
        ...
        const fields = (0, tenants_js_1.fieldMapFor)(args.tenant);
...
async function fillSigninForm(args, accountEmail, password) {
    ...
    await pwLoc.press("Tab").catch(() => page.locator(aid(fields.signinPassword)).first().press("Tab").catch(() => { }));
    await page.waitForTimeout(1500);
    ...
    const loggedIn = await page.evaluate((email) => {
    ...
    if (loggedIn) {
        ...
        if (hasError) {
            ...
            const stillError = await page.evaluate(() => /something went wrong/i.test(document.body.innerText || "")).catch(() => false);
            if (stillError) {
...
async function typeIntoVisible(page, selector, value) {
...
async function typeIntoWithFallback(page, selectors, value) {
 ...
 */
async function clickWithForceFallback(page, selector, steps, label) {
    try {
...
async function clickWithFallbackSelectors(page, selectors, steps, label) {
    for (const sel of selectors) {
        const el = page.locator(sel).first();
        if (await el.isVisible({ timeout: 2000 }).catch(() => false)) {
            try {
