...
exports.generateWorkdayPassword = generateWorkdayPassword;
exports.getOrCreateCredential = getOrCreateCredential;
...
function getMasterKey() {
    ...
    if (!hex) {
        throw new Error("WORKDAY_CREDENTIAL_KEY env var is not set. Generate with `node -e \"console.log(require('crypto').randomBytes(32).toString('hex'))\"` and add to Vercel + Railway env.");
    }
...
function generateWorkdayPassword() {
    ...
    const required = [
        upper[(0, crypto_1.randomInt)(0, upper.length)],
        lower[(0, crypto_1.randomInt)(0, lower.length)],
        digits[(0, crypto_1.randomInt)(0, digits.length)],
        special[(0, crypto_1.randomInt)(0, special.length)],
    ];
    const rest = [];
    for (let i = 0; i < 16; i++) {
        rest.push(all[(0, crypto_1.randomInt)(0, all.length)]);
    }
 ...
 */
async function getOrCreateCredential(userId, tenantHost, applicationEmail) {
    ...
    const id = `wcd_${(0, crypto_1.randomBytes)(12).toString("base64url")}`;
    await db.execute({
...
async function markCredentialUsed(userId, tenantHost) {
    const db = (0, db_1.getDb)();
    await db.execute({
