...
exports.runWorkdayWizard = runWorkdayWizard;
...
function aid(automationId) {
    return `[data-automation-id="${automationId}"]`;
}
function escape(s) {
    return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
async function clearAndType(page, loc, value) {
...
async function detectStep(page) {
    ...
    if (!m)
        return { current: -1, total: -1, rawText: rawText.slice(0, 200) };
...
async function detectStepType(page, stepNumber, totalSteps) {
    ...
    if (signals.hasLegalName || (signals.hasAddress && signals.hasCountry))
        return "my-information";
    if (signals.hasFileUpload)
        return "my-experience";
    if (signals.dropdownCount >= 3)
        return "application-questions";
    if (signals.checkboxCount > 0 && signals.dropdownCount <= 2)
        return "voluntary-disclosures";
    if (signals.dropdownCount >= 2)
        return "application-questions";
    return "unknown";
}
async function typeIntoFormField(page, formFieldAid, value) {
...
async function selectFormFieldDropdown(page, formFieldAid, optionText) {
    const triggers = [
...
async function selectFirstDropdownOption(page, formFieldAid) {
    ...
    if (fdSelected > preSelectedCount)
        return true;
    const postBtnText = (String(await trigger.textContent().catch(() => ""))).trim();
    if (postBtnText !== preBtnText && postBtnText.length > 0)
        return true;
    return false;
}
/**
 ...
 */
async function selectMultiselect(page, formFieldAid, preferText) {
...
async function fillSearchableDropdown(page, formFieldAid, searchTexts, steps) {
    ...
    if (await promptIcon.isVisible({ timeout: 1500 }).catch(() => false)) {
        ...
        const postClickDump = await page.evaluate((aid) => {
            ...
            const btns = Array.from(field.querySelectorAll("button, [role='button']")).map(b => {
                ...
                const aid2 = b.getAttribute("data-automation-id") || "";
                return `btn[${aid2}|${txt}]`;
            });
...
async function selectDropdownViaKeyboard(page, formFieldAid, preferText) {
    ...
    await page.waitForTimeout(300);
    const selected = await page.locator(`[data-automation-id="${formFieldAid}"] [data-automation-id="selectedItem"]`).count().catch(() => 0);
    if (selected > preSelectedCount)
        return true;
    // For regular dropdowns (non-multiselect), check if button text actually changed
    const postBtnText = (String(await trigger.textContent().catch(() => ""))).trim();
    if (postBtnText !== preBtnText && postBtnText.length > 0 && !/select|choose|--/i.test(postBtnText))
        return true;
    return false;
}
async function dumpPageFields(page) {
...
async function clickNextWithFallback(page, steps) {
...
// Step fillers
// ---------------------------------------------------------------------------
async function fillWorkdayDate(page, fieldSuffix, steps) {
    ...
    if (await sel.isVisible({ timeout: 1000 }).catch(() => false)) {
        await sel.selectOption(month).catch(() => sel.selectOption({ index: parseInt(month) }).catch(() => { }));
    }
    else {
...
async function fillMyInformation(page, applicant, tenant, steps) {
    ...
    for (const sa of sourceAids) {
        if (await selectMultiselect(page, sa, "Internet")) {
            sourceFilled = true;
            break;
        }
    }
    ...
    if (!sourceFilled) {
        ...
        if (!sourceFilled)
            sourceFilled = await selectFirstDropdownOption(page, "formField-source");
    }
    if (!sourceFilled) {
      ...
      return Array.from(document.querySelectorAll('[data-automation-id^="formField-"]')).filter(function(el){
        ...
        for (const msf of msFields) {
            if (msf.aid === "formField-countryPhoneCode")
                continue;
          ...
          return { aid: el.getAttribute('data-automation-id') || '', text: (el.innerText || el.textContent || '').trim().slice(0, 200) };
            ...
            for (const dd of allDropdowns) {
                if (/hear|source|how did you/i.test(dd.text)) {
                    ...
                    if (!sourceFilled)
                        sourceFilled = await selectMultiselect(page, dd.aid, "Internet");
                    if (sourceFilled)
                        break;
                }
            }
    ...
    if (!sourceFilled) {
        ...
        if (!sourceFilled)
            sourceFilled = await selectDropdownViaKeyboard(page, "formField-source");
        if (sourceFilled)
            steps.push("workday-wizard: filled source via keyboard");
    }
    ...
    if (!sourceFilled) {
      ...
      return 'hasBtn=' + hasBtn + ' hasPrompt=' + hasPrompt + ' hasInput=' + hasInput + ' hasMSC=' + hasMSC + ' hasSelected=' + hasSelected + ' btnText=' + btnText + ' html=' + html;
        ...
        steps.push("workday-wizard: source-diag: " + srcDiag.slice(0, 600));
    }
    ...
    if (applicant.firstName) {
        ...
        if (!nameFilled)
            await typeIntoFormField(page, "formField-firstName", applicant.firstName);
    }
    if (applicant.lastName) {
        const nameFilled = await typeIntoFormField(page, "formField-legalName--lastName", applicant.lastName);
        if (!nameFilled)
            await typeIntoFormField(page, "formField-lastName", applicant.lastName);
    }
    ...
    if (applicant.city)
        await typeIntoFormField(page, "formField-city", applicant.city);
    if (applicant.usState)
        await selectFormFieldDropdown(page, "formField-countryRegion", applicant.usState);
    ...
    const emailFilled = await typeIntoFormField(page, "formField-emailAddress", applicant.email || "");
    if (!emailFilled) {
    ...
    if (!phoneTypeFilled) {
        ...
        if (!phoneTypeFilled)
            phoneTypeFilled = await selectFirstDropdownOption(page, "formField-phoneDeviceType");
    }
    if (!phoneTypeFilled) {
        phoneTypeFilled = await selectDropdownViaKeyboard(page, "formField-phoneType", "Mobile");
        if (!phoneTypeFilled)
            phoneTypeFilled = await selectDropdownViaKeyboard(page, "formField-phoneDeviceType", "Mobile");
        if (phoneTypeFilled)
            steps.push("workday-wizard: filled phone type via keyboard");
    }
    if (!phoneTypeFilled) {
        ...
        for (const pf of phoneFields) {
            if (pf.hasSelect) {
                ...
                if (await sel.isVisible({ timeout: 1500 }).catch(() => false)) {
                    await sel.selectOption({ label: "Mobile" }).catch(() => sel.selectOption({ index: 1 }).catch(() => { }));
    ...
    if (!phoneTypeFilled) {
      ...
      return Array.from(document.querySelectorAll('[data-automation-id^="formField-"]')).filter(function(el){
        ...
        for (const uf of allUnfilled) {
            ...
            if (!phoneTypeFilled)
                phoneTypeFilled = await selectDropdownViaKeyboard(page, uf.aid, "Mobile");
            if (phoneTypeFilled) {
    ...
    if (applicant.phone)
        await typeIntoFormField(page, "formField-phoneNumber", applicant.phone.replace(/\D/g, ""));
    ...
    for (const ri of remainingInputs) {
        ...
        let value = "N/A";
        if (/name|email|refer/i.test(label))
            value = "Job Board";
    ...
    for (const uf of myInfoUnfilledDropdowns) {
        ...
        if (/hear|source|how did/i.test(text)) {
            for (const label of ["Internet", "Website", "Job Board", "Online"]) {
                ...
                if (filled) {
                    method = `selectFormFieldDropdown("${label}")`;
                    break;
                }
            }
            if (!filled) {
                filled = await selectMultiselect(page, uf.aid, "Internet");
                if (filled)
                    method = "selectMultiselect(Internet)";
            }
        }
        if (!filled) {
            filled = await selectFirstDropdownOption(page, uf.aid);
            if (filled)
                method = "selectFirstDropdownOption";
        }
        if (!filled) {
            filled = await selectMultiselect(page, uf.aid);
            if (filled)
                method = "selectMultiselect(any)";
        }
        if (!filled) {
            filled = await selectDropdownViaKeyboard(page, uf.aid);
            if (filled)
                method = "selectDropdownViaKeyboard";
        }
        if (filled) {
        ...
        if (!filled && /hear|source|how did/i.test(text)) {
            ...
            if (filled)
                steps.push(`workday-wizard: catch-all filled source via fillSearchableDropdown (${uf.aid})`);
        }
    }
...
async function fillMyExperience(page, applicant, resumePath, steps) {
    ...
    if (await jobTitleField.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await roleDescField.isVisible({ timeout: 1500 }).catch(() => false)) {
            ...
            if (!rdVal) {
                ...
                await roleDescField.pressSequentially("Developed software solutions and collaborated with cross-functional teams to deliver projects on time.", { delay: 30 });
            }
        }
    ...
    let fosFilled = await fillSearchableDropdown(page, "formField-fieldOfStudy", ["Computer Science", "Computer Science and Engineering", "Information Technology"], steps);
    if (!fosFilled)
        await selectFormFieldDropdown(page, "formField-fieldOfStudy", "Computer");
    let degreeFilled = await selectFormFieldDropdown(page, "formField-degree", "Bachelor");
    if (!degreeFilled)
        degreeFilled = await selectFormFieldDropdown(page, "formField-degree", "Bachelor's Degree");
    if (!degreeFilled)
        degreeFilled = await selectFirstDropdownOption(page, "formField-degree");
    if (!degreeFilled)
        degreeFilled = await fillSearchableDropdown(page, "formField-degree", ["Bachelor", "Bachelor's", "BS", "B.S."], steps);
    if (!degreeFilled)
        degreeFilled = await selectDropdownViaKeyboard(page, "formField-degree", "Bachelor");
    ...
    if (await fromField.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        if (await input.isVisible({ timeout: 500 }).catch(() => false)) {
            ...
            if (!val) {
                await input.click().catch(() => { });
                await input.pressSequentially("2023", { delay: 60 });
            }
        }
    }
    ...
    if (await toField.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        if (await input.isVisible({ timeout: 500 }).catch(() => false)) {
            ...
            if (!val) {
                await input.click().catch(() => { });
                await input.pressSequentially("2026", { delay: 60 });
            }
        }
    }
    ...
    if (await fyaField.isVisible({ timeout: 500 }).catch(() => false)) {
        ...
        if (!v) {
            await fyaField.click().catch(() => { });
            await fyaField.pressSequentially("2023", { delay: 60 });
        }
    }
    ...
    if (await lyaField.isVisible({ timeout: 500 }).catch(() => false)) {
        ...
        if (!v) {
            await lyaField.click().catch(() => { });
            await lyaField.pressSequentially("2026", { delay: 60 });
        }
    }
    ...
    if (await langField.isVisible({ timeout: 500 }).catch(() => false)) {
        ...
        if (!langFilled)
            langFilled = await selectFirstDropdownOption(page, "formField-language");
        if (!langFilled)
            await fillSearchableDropdown(page, "formField-language", "English", steps);
    }
    ...
    for (const pAid of profFields) {
        ...
        if (!filled)
            filled = await selectFormFieldDropdown(page, pAid, "Intermediate");
        if (!filled)
            await selectFirstDropdownOption(page, pAid);
    }
    ...
    for (const ef of emptyFields) {
        ...
        let value = "";
        if (/company/i.test(label))
            value = "University of Maryland";
        else if (/job title|role|position/i.test(label))
            value = applicant.currentTitle || "Software Engineer Intern";
        else if (/location|city/i.test(label))
            value = applicant.city || "Denver, CO";
        else if (/gpa/i.test(label))
            value = "3.5";
        else
            continue;
    ...
    for (const uf of unfilledDropdowns) {
        ...
        if (filled)
            steps.push(`workday-wizard: catch-all filled dropdown "${uf.text}" (${uf.aid})`);
    }
...
async function fillApplicationQuestions(page, tenant, steps) {
    ...
    for (const ff of fields) {
        const preferText = (0, tenants_js_1.answerForQuestion)(tenant, ff.text);
    ...
    for (const ti of textInputs) {
        ...
        if (await input.isVisible({ timeout: 500 }).catch(() => false)) {
            ...
            let value = "N/A";
            if (/salary|compensation|pay|wage/i.test(label))
                value = "70000";
            else if (/year|experience/i.test(label))
                value = "0";
            else if (/date|when|graduation/i.test(label))
                value = "05/2026";
            await input.click().catch(() => { });
    ...
    for (let rescan = 0; rescan < 3; rescan++) {
        await page.waitForTimeout(500);
        const newDropdowns = (await page.evaluate(`(function(){
      return Array.from(document.querySelectorAll('[data-automation-id^="formField-"]')).filter(function(el){
        var r = el.getBoundingClientRect();
        if (r.width <= 0 || r.height <= 0) return false;
    ...
    for (const ti of lateTextInputs) {
        ...
        if (await input.isVisible({ timeout: 500 }).catch(() => false)) {
            ...
            if (/salary|compensation|pay|wage/i.test(label))
                ...
                value = "0";
            else if (/date|when|start|available/i.test(label))
                value = "05/2026";
            await input.click().catch(() => { });
            await input.pressSequentially(value, { delay: 30 }).catch(() => { });
        }
...
async function fillVoluntaryDisclosures(page, applicant, steps) {
    ...
    for (const ff of genderFields) {
        ...
        if (/consent|terms|conditions|acknowledge|agree|certif/i.test(label))
            preferText = "Yes";
        else if (/gender/i.test(label))
            preferText = "Decline to Self Identify";
        else if (/race|ethnic/i.test(label))
            preferText = "Decline to Self Identify";
        else if (/veteran/i.test(label))
            preferText = "not a veteran";
        else if (/disability|disab/i.test(label))
            preferText = "prefer not";
        else if (/citizen/i.test(label))
            preferText = "United States";
        else if (/nationality/i.test(label))
            preferText = "United States";
        else if (/marital/i.test(label))
            preferText = "Single";
        else if (/engage|contract|business.*relat|current.*role|worked.*for|employed.*by|relative|family.*member|previous.*worker/i.test(label))
            preferText = "No";
        if (ff.isMultiselect) {
    ...
    for (const uf of vdUnfilled) {
        ...
        if (!filled)
            filled = await selectFirstDropdownOption(page, uf.aid);
        if (!filled)
            filled = await fillSearchableDropdown(page, uf.aid, preferText ? [preferText] : ["United States"], steps);
        if (filled)
            steps.push(`workday-wizard: VD fallback filled "${uf.text}" (${uf.aid})`);
    }
    ...
    for (const radio of yesRadios) {
        if (await radio.isVisible({ timeout: 500 }).catch(() => false)) {
            ...
            if (!checked)
                await radio.check({ timeout: 2000 }).catch(() => { });
        }
    }
    ...
    for (let i = 0; i < combined.length; i++) {
        ...
        const label = ti.label.toLowerCase();
        let value = "";
        if (/\bname\b|signature/i.test(label) && !/date/i.test(label))
            value = `${applicant.firstName} ${applicant.lastName}`;
        else if (/\bdate\b|today/i.test(label) || ti.aid.toLowerCase().includes("date"))
    ...
    for (const re of remainingEmpty) {
        ...
        let val = "N/A";
        if (/graduation.*date|expected.*date|anticipated.*date/i.test(lbl))
            val = "05/2026";
        else if (/\bdate\b/i.test(lbl))
            val = new Date().toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "numeric" });
        else if (/salary|compensation|pay/i.test(lbl))
            val = "70000";
    ...
    if (textFilled === 0 || !combined.some((ti) => /date/i.test(ti.label))) {
        ...
        for (const aid of dateContainerAids) {
            ...
            if (await monthEl.isVisible({ timeout: 500 }).catch(() => false)) {
                const tag = await monthEl.evaluate((el) => el.tagName).catch(() => "");
                if (tag === "INPUT") {
                ...
                await monthSel.selectOption(String(useMonth + 1).padStart(2, "0")).catch(() => monthSel.selectOption({ index: useMonth + 1 }).catch(() => { }));
            }
            // Day input
        ...
        if (!dateFilled) {
            ...
            if (await monthWidget.isVisible({ timeout: 500 }).catch(() => false)) {
                const tag = await monthWidget.evaluate((el) => el.tagName).catch(() => "");
                // Check if this date widget is inside a DOB container
                const s2IsDob = await monthWidget.evaluate((el) => {
                    for (let p = el; p; p = p.parentElement) {
                        const aid = p.getAttribute("data-automation-id") || "";
                        if (/dateOfBirth|dob/i.test(aid))
                            return true;
                        const text = (p.textContent || "").toLowerCase();
                        if (/date of birth/i.test(text) && text.length < 200)
                            return true;
                    }
                    return false;
                }).catch(() => false);
                ...
                if (tag === "INPUT") {
                    ...
                    await monthWidget.click().catch(async () => {
                        await monthWidget.evaluate((e) => { e.focus(); e.select(); }).catch(() => { });
                    });
                    await page.waitForTimeout(200);
          ...
          return !filled;
            ...
            for (const fieldAid of dateByLabel) {
                ...
                if (await monthW.isVisible({ timeout: 500 }).catch(() => false)) {
                    const tag = await monthW.evaluate((el) => el.tagName).catch(() => "");
                    if (tag === "INPUT") {
...
// Wizard driver
// ---------------------------------------------------------------------------
async function runWorkdayWizard(args) {
    ...
    for (let iter = 0; iter < MAX_WIZARD_ITERATIONS; iter++) {
        if (deadlineMs() <= 0)
            return { success: false, error: "workday-wizard-budget-exhausted", steps };
        ...
        if (step.current === step.total) {
            ...
            const url = page.url();
            const bodyText = (await page.evaluate(`document.body.innerText`).catch(() => "")).slice(0, 1000);
            const success = /\/(completed|confirmation)\/application/i.test(url) ||
        ...
        if (stepType === "my-information")
            await fillMyInformation(page, applicant, tenant, steps);
        else if (stepType === "my-experience")
            await fillMyExperience(page, applicant, resumePath, steps);
        else if (stepType === "application-questions")
            await fillApplicationQuestions(page, tenant, steps);
        else if (stepType === "voluntary-disclosures")
            await fillVoluntaryDisclosures(page, applicant, steps);
        else if (stepType === "self-identify")
            await fillVoluntaryDisclosures(page, applicant, steps);
        else
            steps.push(`workday-wizard: unknown step type "${stepType}" at step ${step.current}, attempting Next`);
        ...
        return Array.from(errs).map(function(e){ return (e.innerText || e.textContent || '').trim(); }).filter(Boolean).join(' | ').slice(0, 300);
            ...
            if (stuckErrors)
                steps.push(`workday-wizard: stuck-step errors: ${stuckErrors}`);
        ...
        return Array.from(errs).map(function(e){ return (e.innerText || e.textContent || '').trim(); }).filter(Boolean).join(' | ').slice(0, 300);
            ...
            if (errors)
                steps.push(`workday-wizard: validation errors: ${errors}`);
