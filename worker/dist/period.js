...
exports.readQuotaWithLazyReset = readQuotaWithLazyReset;
 ...
 */
async function readQuotaWithLazyReset(userId) {
    ...
    if (new Date(row.monthlyAppResetAt) < periodStart) {
        ...
        monthlyAppCount = Number(sumRes.rows?.[0]?.s ?? 0);
        await db.execute({
            sql: `UPDATE User
