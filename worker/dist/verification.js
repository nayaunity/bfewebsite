...
exports.waitForVerificationCode = waitForVerificationCode;
exports.extractVerificationCode = extractVerificationCode;
 ...
 */
async function waitForVerificationCode(applicationEmail, maxWaitMs = 240_000, pollIntervalMs = 1_500) {
    ...
    while (Date.now() - startTime < maxWaitMs) {
        ...
        if (result.rows && result.rows.length > 0) {
            ...
            const textBody = row.textBody || "";
            const htmlBody = row.htmlBody || "";
    ...
    console.log(`[Verification] No usable code after ${elapsedMs}ms (polls: ${pollCount}, inbound emails seen: ${inboundEmailCountInWindow})`);
...
function extractVerificationCode(text, html) {
    if (html) {
        ...
        const sixDigitTag = html.match(/<(?:h1|h2|h3|strong|b|code|td|span)[^>]*>\s*(\d{6})\s*<\/(?:h1|h2|h3|strong|b|code|td|span)>/i);
        if (sixDigitTag)
            return sixDigitTag[1];
        // Strategy 1c: 8-char alphanumeric inside td/span (custom email templates)
        const tagWrapped = html.match(/<(?:td|span)[^>]*>\s*([A-Za-z0-9]{8})\s*<\/(?:td|span)>/i);
        if (tagWrapped)
            return tagWrapped[1];
    }
