...
exports.applyToJob = void 0;
exports.discoverJobsFromCatalog = discoverJobsFromCatalog;
exports.discoverJobs = discoverJobs;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const apply_engine_1 = require("./apply-engine");
Object.defineProperty(exports, "applyToJob", { enumerable: true, get: function () { return apply_engine_1.applyToJob; } });
...
const INTERN_HARD_NEGATIVE_RX = /\bintern\s+program\s+manager\b|\bintern\s+coordinator\b|\bmanages?\s+interns?\b/i;
const INTERN_SOFT_NEGATIVE_RX = /\binternal\b|\binternational\b/i;
const INTERN_STANDALONE_RX = /\b(intern|internship|co-?op)\b/i;
function looksLikeIntern(title) {
    if (!INTERN_TITLE_RX.test(title))
        return false;
    if (INTERN_HARD_NEGATIVE_RX.test(title))
        return false;
 ...
 */
async function discoverJobsFromCatalog(companySlug, targetRole, seekingInternship = false) {
 ...
 */
function parseRoles(targetRole) {
    return targetRole
        .split(/[,;]+/)
        .map((r) => r.trim())
        .filter((r) => r.length > 0);
 ...
 */
async function discoverViaGreenhouseAPI(careersUrl, roles, log) {
    ...
    try {
        ...
        const jobs = matched.slice(0, 15).map(job => ({
            title: job.title,
 ...
 */
async function discoverJobs(careersUrl, targetRole) {
    const log = { steps: [] };
    const roles = parseRoles(targetRole);
    ...
    const context = await (0, apply_engine_1.createStealthContext)();
    ...
    try {
        ...
        for (let i = 0; i < pageLinks.length; i += BATCH_SIZE) {
            ...
            if (i > 0)
                log.steps.push(`Processing batch ${Math.floor(i / BATCH_SIZE) + 1}...`);
        ...
        const jobUrls = converted.filter((j) => {
            ...
            if (url.includes("greenhouse.io/") && url.includes("/jobs/"))
                return true;
            if (/\/(jobs|positions|listing|careers)\/.+/.test(url) && !url.includes("/search") && !url.includes("/results"))
                return true;
            return false;
        });
 ...
 */
async function matchJobsWithClaude(links, roles, log) {
 ...
 */
function appendSearchParam(url, query) {
 ...
 */
async function trySearchInput(page, query, log) {
    const selectors = [
        'input[type="search"]',
 ...
 */
async function tryNextPage(page, roles, log) {
