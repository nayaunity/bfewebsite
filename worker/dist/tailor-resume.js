...
exports.fetchJobDescription = fetchJobDescription;
exports.canTailorResume = canTailorResume;
exports.incrementTailorQuota = incrementTailorQuota;
...
Output the complete HTML resume now:`;
    ...
    const tailoredPath = (0, path_1.join)((0, os_1.tmpdir)(), `tailored-${Date.now()}.pdf`);
    try {
        const browser = await (0, apply_engine_1.getBrowser)();
 ...
 */
async function canTailorResume(userId, subscriptionTier) {
    ...
    const count = result.rows?.[0]?.monthlyTailorCount || 0;
    return count < 1;
}
/**
 * Increment the tailor count. Call only after a successful tailored apply.
 */
async function incrementTailorQuota(userId) {
...
async function incrementTailorCount(userId) {
    const db = (0, db_1.getDb)();
    await db.execute({
