...
exports.fillCommonGates = fillCommonGates;
exports.ashbyIdentityFill = ashbyIdentityFill;
exports.leverIdentityFill = leverIdentityFill;
/**
 ...
 */
async function selectDropdown(target, comboboxNamePattern, optionName, steps) {
    try {
        ...
        steps.push(`Gate dropdown failed ${comboboxNamePattern.source}: ${e instanceof Error ? e.message : String(e)}`);
    }
}
/**
 ...
 */
async function clickRadio(target, labelPattern, choice, steps) {
    try {
 ...
 */
async function checkCheckbox(target, labelPattern, steps) {
    try {
        const box = target.getByRole("checkbox", { name: labelPattern }).first();
        if (!(await box.isVisible({ timeout: 150 }).catch(() => false)))
            return;
 ...
 */
async function fillCommonGates(target, applicant, steps) {
    // --- Work authorization ---
 ...
 */
async function ashbyIdentityFill(page, applicant, steps) {
    const fields = [
 ...
 */
async function leverIdentityFill(page, applicant, steps) {
    const fields = [
